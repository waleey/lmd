...
void mDOM::GenerateLogicals()
{

    std::cerr << "OMSimMDOM::GenerateLogicals is called " << std::endl;
    SetPMTPositions();
    SetLEDPositions();
    ...
    std::cerr << "OMSimMDOM::supported structure generated" << std::endl;
    ...
    std::cerr << "OMSimMDOM::reflector solid generated" << std::endl;
    ...
    std::cerr << "OMSimMDOM::reflector cone generated" << std::endl;
    ...
    std::cerr << "OMSimMDOM::led flashers generated" << std::endl;

    //Logicals
    lGelLogical = new G4LogicalVolume(lGelSolid,
        mData->GetMaterial("argGel"),
        "Gelcorpus logical");
    lSupStructureLogical = new G4LogicalVolume(lSupStructureSolid,
        mData->GetMaterial("NoOptic_Absorber"),
        "TubeHolder logical");
    lGlassLogical = new G4LogicalVolume(lGlassSolid,
    ...
    lRefConePolarLogical = new G4LogicalVolume(lRefConePolarSolid,
        mData->GetMaterial("NoOptic_Reflector"),
        "RefConeType1 logical");

    lRefconeEqUpCutLogical = new G4LogicalVolume(lRefConeEqUpCutSolid,
        mData->GetMaterial("NoOptic_Reflector"),
        "RefConeType2 ETEL logical");

    lRefconeEqLoCutLogical = new G4LogicalVolume(lRefConeEqLoCutSolid,
        mData->GetMaterial("NoOptic_Reflector"),
        "RefConeType3 ETEL logical");
    lLEDHoleAirLogical = new G4LogicalVolume (lLEDHoleAirSolid, 
        mData->GetMaterial("Ri_Vacuum"), 
        "LED hole");
    lLEDGlassTopLogical = new G4LogicalVolume (lLEDGlassTopSolid, 
        mData->GetMaterial("RiAbs_Glass_Vitrovex"), 
        "LED Glass top logical");
    lLEDLogical = new G4LogicalVolume (lLEDSolid, 
        mData->GetMaterial("Ri_Glass_LED"), 
        "LED logical");

    std::cerr << "OMSimMDOM::logicals generated" << std::endl;
}
...
void mDOM::Construction()
{
    ...
    new G4PVPlacement(0, G4ThreeVector(0, 0, 0), lGelLogical, "Gel_physical", lGlassLogical, false, 0);
    ...
    AppendComponent(lGlassLogical->GetSolid(), lGlassLogical, G4ThreeVector(0, 0, 0), G4RotationMatrix(), "mDOM");
...
{
    std::cerr << "mDOM::SupportStructure is called " << std::endl;
